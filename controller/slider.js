const model = require("../models/Slider.js");
const asyncHandler = require("../middleware/asyncHandle.js");

exports.create = asyncHandler(async (req, res, next) => {
    try {
        const { price } = req.body;
        const invoiceCreate = await model.create({ price: price, userId: req.userId });
        return res
            .status(200)
            .json({ success: true, price, data: invoiceCreate });
    } catch (error) {
        console.error(error);
...
exports.update = asyncHandler(async (req, res, nesxt) => {
    try {
        const updatedData = {
            ...req.body,
        };
